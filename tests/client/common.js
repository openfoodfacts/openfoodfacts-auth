...
export const cc = urlParams.get("cc");
export const keycloak = 'http://auth.openfoodfacts.localhost:5600/realms/openfoodfacts';
...
export function login() {
  ...
  const url = `${keycloak}/protocol/openid-connect/auth?response_type=code&client_id=${clientId}&redirect_uri=${encodeURIComponent(
    redirectUri()
  )}&scope=openid+profile+offline_access&state=${nonce}&ui_locales=${lang}&cc=${cc ?? 'world'}`;
  window.location = url;
}
...
export async function pkceLogin() {
  ...
  const url = `${keycloak}/protocol/openid-connect/auth?response_type=code&client_id=${pkceClientId}&redirect_uri=${encodeURIComponent(
    redirectUri(true)
  )}&scope=openid+profile+offline_access&state=${nonce}&ui_locales=${lang}&cc=${cc ?? 'world'}&code_challenge=${codeChallenge}&code_challenge_method=S256`;
  window.location = url;
}
